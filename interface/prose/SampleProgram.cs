...
namespace Transformation.Text
{
    ...
    internal static class SampleProgram
    {
        ...
        {   
            ...
            string test_filePath = args[0];
            string filename = args[1];

            string jsonContent = File.ReadAllText(test_filePath);

            // Parse the JSON string into a JObject
            JObject jsonObject = JObject.Parse(jsonContent);

            // Get the "Examples" array from the JObject
            JArray examplesArray = (JArray)jsonObject["Examples"];

            LearnFormat(examplesArray, filename);
        ...
        /// </summary>
        private static void LearnFormat(JArray dataArray, string filename)
        {
            var session = new Session();

            // Iterate over the first two examples
            for (int i = 0; i < 2; i++)
            {
                // Get the "Input" array from the current example
                string inputValue = (string)dataArray[i]["Input"][0];
                string outputValue = (string)dataArray[i]["Output"][0];

                // create new example object
                Example example = new Example(new InputRow(inputValue),outputValue);
                session.Constraints.Add(example);
            }

            // // Examples are given as an Example object which takes an input and output.
            // session.Constraints.Add(
            //     new Example(new InputRow("Consectetur"), "C"),
            //     new Example(new InputRow("Nihil Impedit"), "NI")
            // );
            ...
            if (topRankedProgram == null)
            {
                ...
                outputdata["result"] = new string[] {};
                string jsonString = JsonConvert.SerializeObject(outputdata, Formatting.Indented);
                string filePath = "../../../../output/prose/foofah/" + filename + ".json";
            ...
            {
                // create the testing array
                string[] test = new string[dataArray.Count];
                string[] result = new string[dataArray.Count];

                for (int i = 0; i < dataArray.Count; i++)
                {
                    test[i] = (string)dataArray[i]["Input"][0];
...
//
//
// Specific function provided by Prose team
//
//
//
    ...
    new string[] { "USA", "2010", "100" },
    new string[] { "USA", "2011", "" }
};

// Create an instance of InputRow using the input data
            InputRow inputRow = new InputRow(inputData);

            // Examples are given as an Example object which takes an input and output.
            session.Constraints.Add(
                new Example(inputRow,new string[] { "3099", "905", " AUST 4WD CUST ACT", "NO.14", "Full Copies", "6.7839", "2" })
            );
            // Given just the examples, the best program is returned
            Program topRankedProgram = session.Learn();
        ...
        private static void LearnNormalizePhoneNumber()
        {
            ...
            session.Constraints.Add(
                new Example(new InputRow("USA", "2010", "100" ), new InputRow("USA", "2010", "100")),
                new Example(new InputRow("USA", "2011", ""), new InputRow("USA", "2011", "100"))
            );
            Program topRankedProgram = session.Learn();
            ...
            {
                foreach (var phoneNumber in new[] { "USA", "2011", "", "(425) 777 3333" })
                {
